...
beforeAll(async () => {
  // Create two users to test user isolation
...
describe('createWallet', () => {
  ...
  it('stores the address lowercased', async () => {
    await createWallet({ userId: user1Id, alias: 'Test', address: '0xABCDEF', chain: 'bsc' });
...
describe('getWallets', () => {
  it('returns only wallets belonging to the given user', async () => {
    await createWallet({ userId: user1Id, alias: 'W1', address: '0x111', chain: 'ethereum' });
    await createWallet({ userId: user2Id, alias: 'W2', address: '0x222', chain: 'polygon' });
...
describe('getWalletById', () => {
  it('returns the wallet for the correct user', async () => {
    await createWallet({ userId: user1Id, alias: 'Mine', address: '0x333', chain: 'ethereum' });
  ...
  it('returns undefined for a different user', async () => {
    await createWallet({ userId: user1Id, alias: 'NotYours', address: '0x444', chain: 'bsc' });
...
describe('updateWallet', () => {
  it('updates the alias for the correct user', async () => {
    await createWallet({ userId: user1Id, alias: 'Old', address: '0x555', chain: 'ethereum' });
    const [{ id }] = await db('wallets').where('alias', 'Old');

    await updateWallet(id, user1Id, { alias: 'New', address: '0x555', chain: 'ethereum' });
  ...
  it('does not update a wallet belonging to a different user', async () => {
    await createWallet({ userId: user1Id, alias: 'Original', address: '0x666', chain: 'polygon' });
    const [{ id }] = await db('wallets').where('alias', 'Original');

    await updateWallet(id, user2Id, { alias: 'Hacked', address: '0x666', chain: 'polygon' });
...
describe('deleteWallet', () => {
  it('deletes the wallet for the correct user', async () => {
    await createWallet({ userId: user1Id, alias: 'ToDelete', address: '0x777', chain: 'bsc' });
  ...
  it('does not delete a wallet belonging to a different user', async () => {
    await createWallet({ userId: user1Id, alias: 'Protected', address: '0x888', chain: 'ethereum' });
